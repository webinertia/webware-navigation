...
**Branch**: `001-webware-tools-alignment` | **Date**: 2026-08-18 | **Spec**: `specs/001-webware-tools-alignment/spec.md`
...
**Primary Dependencies**: `laminas/laminas-view ^3.0`, `mezzio/mezzio-router ^3.15`,
`psr/container ^2.0`, `psr/http-message ^2.0`, `psr/http-server-* ^1.0`; dev:
PHPUnit `^13.3.0`, Infection `^0.34.1`, PHPBench `^1.7`, roave/backward-compatibility-check
`^8.21.0`, `laminas/laminas-permissions-acl ^2.16`, `webware/webware-tools ^0.1.x-dev`

**Storage**: N/A (navigation has no database; reusable workflow DB steps skipped when `db-image`
omitted)
...
**Project Type**: PHP library (ACL-aware navigation middleware, container, Laminas view helper)

**Performance Goals**: N/A

**Constraints**: CI matrix `["8.4", "8.5"]`; `config.platform.php` = `8.4.99`; `min-msi` and
`min-covered-msi` = 95; integration tests run in-process, no external services
...
test/
├── unit/                                  # WebwareTest\Navigation\
└── integration/                           # WebwareTestIntegration\Navigation\
```
...
Navigation is already aligned; current repo state is compared file-by-file against the reference
(webware-mailer). Only drift found gets fixed.

### Phase 1 — composer.json

- `require.php`: `~8.4.1 || ~8.5.0`. Do not add `8.6.0-dev`.
- `require-dev`: PHPUnit `^13.3.0`; Infection `^0.34.1`, PHPBench `^1.7`,
  roave/backward-compatibility-check `^8.21.0`; keep package deps,
  `webware/webware-tools`, `roave/security-advisories`.
- `config.platform.php`: `8.4.99`.
- `autoload-dev`: `WebwareTest\Navigation\` → `test/unit/`;
...
- `.github/workflows/continuous-integration.yml`: wrapper mirroring the reference with package
  ...
  - `secrets: inherit`
  - `with`: `php-versions: '["8.4", "8.5"]'`, `run-integration: true`, `enable-codecov: true`,
    `enable-infection: true`, `coverage-php-version: "8.5"`, `min-msi: "95"`,
    `min-covered-msi: "95"`; DB inputs omitted (in-process integration tests).
