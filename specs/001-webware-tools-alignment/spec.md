...
**Input**: User description: "Align webware-navigation's CI/CD pipeline and dev tooling with the
...
| Parameter | webware-navigation value |
|---|---|
...
| Integration container | none (integration tests run in-process, no external services) |
| Test autoload namespaces | `WebwareTest\Navigation\` → `test/unit/`, `WebwareTestIntegration\Navigation\` → `test/integration/` |
...
- Integration tests run in-process with no external services, so no container wiring is needed.
...
- **SC-002**: `mago format --check`, `mago lint`, `mago analyze`, `mago guard` report zero
  unbaselined issues.
- **SC-003**: Infection MSI and covered MSI at or above package thresholds (95).
