...
- [ ] T003 Set `autoload-dev` namespaces: `WebwareTest\Navigation\` → `test/unit/`,
  `WebwareTestIntegration\Navigation\` → `test/integration/` in `composer.json`
...
- [ ] T015 Create `.github/workflows/continuous-integration.yml` wrapper calling
  ...
  `secrets: inherit` and package inputs (`php-versions: '["8.4", "8.5"]'`, `run-integration:
  true`, `enable-codecov: true`, `enable-infection: true`, `coverage-php-version: "8.5"`,
